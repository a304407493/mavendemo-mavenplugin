 ...
 * 从MDC中获取部分内容：%d [%X{loginUserId}/%X{req.remoteAddr}/%X{req.id} - %X{req.requestURI}?%X{req.queryString}] %-5p %c{2} - %m%n
 * </pre>
 * @author badqiu
 */
...
//            MDC.put("req.requestURIWithQueryString", request.getRequestURL() + (request.getQueryString() == null ? "" : "?"+request.getQueryString()));
    ...
    public static boolean isPlus(String amt) {
        char fir = amt.charAt(0);//获取字符串的第一个字符
    ...
    public static void main(String[] args) {
    	String test = "-0.01";
    	System.out.println(isPlus(test));
	}
    /**
     * 清除MDC
     */
    ...
    /**
     * 日志输出
     * @param requestURIWithQueryString
     */
